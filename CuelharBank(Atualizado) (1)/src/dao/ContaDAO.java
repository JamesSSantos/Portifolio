...
public class ContaDAO {
	...
	public Conta autenticacao(Conta conta) {
		Conta contRetorno = new Conta();
		Agencia agencia = new Agencia();
		TipoConta tipoConta = new TipoConta();
		String sqlSelect = "SELECT numeroConta, senhaConta, dtAberturaConta, saldoConta, Cliente_idCliente, Agencia_idAgencia, TipoConta_idTipoConta, numeroToken, TipoConta.tipoconta FROM Conta INNER JOIN TipoConta WHERE numeroConta= ? and senhaConta= ? and Agencia_idAgencia = ? and numeroToken= ?";
		...
		try (Connection conn = ConnectionFactory.obtemConexao();
			...
			try (ResultSet rs = stm.executeQuery();) {
				if (rs.next()) {
					...
					tipoConta.setDescricaoTipoConta(rs.getString("Tipoconta.tipoConta"));
					contRetorno.setTipoConta(tipoConta);
